...
namespace API.Repositories
{
    public class ConnectionRepository : IConnectionRepository
    {
        ...
        {
            return await _context.Connections
               .Where(x => x.User.FriendsInvited.Where(x => x.IsConfirmed == true).Select(x => x.FriendId).Contains(userId) 
                    || x.User.FriendsAccepted.Where(x => x.IsConfirmed == true).Select(x => x.UserId).Contains(userId))
               .Select(x => x.ConnectionId)
               .ToListAsync();
        }
